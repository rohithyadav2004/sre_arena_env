"""GRPO reward function for defender training.

GRPOTrainer (TRL >=0.21.0) calls reward functions as:
    fn(prompts, completions, **extra_columns) -> list[float]

Extra Dataset columns (attacker_params, episode_seed) are forwarded by TRL
as keyword arguments, each a list aligned with the completions list.
"""
...
try:
    ...
    from .action_parser import parse_defender_action
except ImportError:
    from server.sre_arena_env_environment import SreArenaEnvironment
    from training.action_parser import parse_defender_action
